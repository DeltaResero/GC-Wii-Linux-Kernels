 ...
 *
 */
static int mi_proc_status_show(struct seq_file *m, void *v)
{
	struct mi_private *priv = (struct mi_private *)v;
	int region;
...
static int mi_proc_status_open(struct inode *inode, struct file *file)
{
	return single_open(file, mi_proc_status_show, NULL); //NULL
}

static const struct file_operations mi_proc_status_fops = {
	.open = mi_proc_status_open,
	.read = seq_read,
	.llseek = seq_lseek,
	.release = seq_release,
};
...
static int mi_probe(struct device *device, struct resource *mem, int irq)
{
	...
	*/
	...
	priv->proc_file = proc_create(dev_name(&pdev->dev),
						 0444, NULL,
						 &mi_proc_status_fops);
	}
#endif /* CONFIG_PROC_FS */
